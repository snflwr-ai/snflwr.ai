...
def _normalize_text(text: str) -> Tuple[str, str]:
    ...
    * **letters_only** strips everything except ``[a-z]`` so substring
    ...
    ``i``), the letters-only form is the *union* of both mappings so
    that substring checks catch either interpretation.
    """
    try:
        lowered = text.lower()
...
_CATEGORY_PATTERNS: Dict[str, List[Tuple[str, str]]] = {
    ...
    # =================================================================
    # VIOLENCE — murder, terrorism, weapons, assault, threats
    # =================================================================
    "VIOLENCE": [
        # -- English --
...
# Words that legitimately contain offensive substrings and should NOT
# trigger the substring-level check.  Checked before flagging.
_FALSE_POSITIVE_ALLOWLIST = frozenset({
    ...
    # contains "crack"
    "cracker",  # as in food — note: "cracker" as slur is in regex
    "firecracker",
    # contains "hell"
    ...
    "scunthorpe",
    # contains "fag"
    "faggot",  # intentionally NOT allowed — keep blocked
    # contains "nit"
    "unit", "ignite", "finite",
    # contains "murder"
    # (no common false positives)
...
class AccountCreationSafetyFilter:
    ...
    def check_field(self, text: str, field_name: str) -> Dict:
        ...
        for category, patterns in _COMPILED_PATTERNS.items():
            ...
            if not matched and text_lower not in _FALSE_POSITIVE_ALLOWLIST:
