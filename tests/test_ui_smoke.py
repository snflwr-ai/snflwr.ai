...
class TestSkipChildProfile:
    ...
    def test_create_account_creates_child_when_not_skipped(
        ...
        mock_auth.authenticate_parent.return_value = (
            True,
            {"parent_id": "parent_001", "session_token": "tok"},
        )
        mock_pm_cls.return_value.create_profile.return_value = MagicMock(
            profile_id="child_001"
        )

        with patch.object(wizard, "_get_owui_admin_token", return_value=""):
            with patch.object(
                wizard, "_create_owui_student_account", return_value=None
            ):
                wizard._create_account()
...
class TestLauncherLoginFlow:
    ...
    def test_successful_login_stores_session_and_transitions(
        ...
        mock_auth.authenticate_parent.return_value = (
            True,
            {"parent_id": "p1", "session_token": "tok"},
        )
        ...
        launcher.action_button.config.assert_called_with(text="Start Snflwr")
...
class TestPlaceholderTranslation:
    ...
    def test_simple_replacement(self):
        assert (
            self._translate("SELECT * FROM t WHERE id = ?")
            == "SELECT * FROM t WHERE id = %s"
        )

    def test_multiple_placeholders(self):
        assert self._translate("WHERE a = ? AND b = ?") == "WHERE a = %s AND b = %s"
...
class TestWizardSessionCleanup:
    ...
    def test_wizard_logs_out_verification_token(
        ...
        with patch("ui.setup_wizard.ProfileManager") as MockPM:
            ...
            mock_auth.authenticate_parent.return_value = (
                True,
                {"parent_id": "parent_001", "session_token": "tok_wizard"},
            )
            MockPM.return_value.create_profile.return_value = MagicMock(
                profile_id="child_001"
            )

            with patch.object(wiz, "_get_owui_admin_token", return_value=""):
                with patch.object(
                    wiz, "_create_owui_student_account", return_value=None
                ):
                    wiz._create_account()
...
def test_launch_parent_dashboard_opens_browser(monkeypatch):
    ...
    launcher = launcher_mod.LauncherWindow.__new__(launcher_mod.LauncherWindow)
