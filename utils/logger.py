...
class PIISanitizer(logging.Filter):
    ...
    _TOKEN_PATTERN = re.compile(r"\b[0-9a-fA-F]{32,}\b")
    _PASSWORD_PATTERN = re.compile(r"(password|passwd|pwd)\s*[=:]\s*\S+", re.IGNORECASE)
    ...
    def _sanitize(self, text: str) -> str:
        ...
        text = self._PASSWORD_PATTERN.sub(
            lambda m: (
                m.group().split("=")[0] + "=[PASSWORD_REDACTED]"
                if "=" in m.group()
...
class SnflwrFormatter(logging.Formatter):
    ...
    def _format_structured(self, record: logging.LogRecord) -> str:
        ...
        log_data["trace"] = {
            k: v for k, v in log_data["trace"].items() if v and v != "-"
        }
    ...
    def log_incident(
        ...
        with self._lock:
            ...
            try:
                ...
                Exception
            ) as e:  # Intentional catch-all: incident logger must not crash
...
class LoggerManager:
    ...
    def _setup_application_logger(self):
        ...
        console_handler.setFormatter(
            SnflwrFormatter(use_color=sys.stdout.isatty(), structured=use_structured)
        )
        console_handler.addFilter(correlation_filter)
        console_handler.addFilter(pii_sanitizer)
