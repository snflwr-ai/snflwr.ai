...
| Severity | Issue | Location |
|----------|-------|----------|
| RESOLVED | Open WebUI Docker tag now parameterized via `WEBUI_DOCKER_TAG` (reviewed default `v0.11.3`); guarded upgrader smoke-tests and rolls back | start_snflwr.sh:~637 |
...
- **`gemma4:31b` tier** (`SNFLWR_ENABLE_GEMMA_31B`) — **CORRECTED 2026-09-12.** The previous entry said 31b and e4b were "near-tied on quality (99.3 vs 98.5)" from a 34-case single run; a 0.8 gap is inside that harness's noise. Three repeats on 48 cases measure **31b 88.8 vs e4b 80.3** (gap 8.5 against a largest within-model spread of 2.1), and a blind judged re-run agrees: composite **95.4 vs 82.9**, pedagogy **1.917 vs 1.708**. 31b is measurably better. The **VRAM ≥ 26 GB gate was also wrong twice**: 31b measures **19.4 GB** resident and fits a 23 GB card, and its stated reason — that 31b cannot co-reside with the ~5 GB guard — no longer holds, because the guard is CPU-pinnable (`SAFETY_MODEL=llama-guard3-cpu:latest`, same weights, 1.9s vs 1.8s, 0% GPU) and was verified running alongside a 17.8 GB model on the card. The real trade is **throughput**: 31b serves ~8 turns/min against e4b's ~30.
