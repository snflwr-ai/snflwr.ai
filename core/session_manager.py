...
class SessionManager:
    ...
    def create_session(
        ...
        with self._session_lock:
            ...
            if (
                ...
                and self.get_active_session(profile_id) is not None
            ):
    ...
    def end_session(self, session_id: str) -> bool:
        ...
        try:
            ...
            logger.warning(
                f"Failed to calculate session duration for {sanitize_log_value(session_id)!r}: {e}"
            )
            duration = None
    ...
    def increment_question_count(self, session_id: str) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Failed to increment question count for session {session_id!r}: {e}"
            )
            return False
        return True
    ...
    def get_session_duration(self, session_id: str) -> Optional[int]:
        ...
        try:
            ...
            logger.warning(
                f"Failed to compute duration for active session {session_id}: {e}"
            )
            return None
    ...
    def _update_last_activity(self, session_id: str, iso_ts: str):
        ...
        try:
            ...
            logger.error(
                f"Failed to update last_activity for session {session_id}: {e}"
            )
            return False
    ...
    def _set_session_duration(self, session_id: str, minutes: int) -> bool:
        ...
        try:
            self.db.execute_write(
                "UPDATE sessions SET duration_minutes = ? WHERE session_id = ?",
                (minutes, session_id),
            )
            return True
        except DB_ERRORS as e:
    ...
    def check_daily_time_limit(self, profile_id: str):
        ...
        today = datetime.now(timezone.utc).date().isoformat()
        rows = self.db.execute_query(
            "SELECT SUM(COALESCE(duration_minutes,0)) as total FROM sessions WHERE profile_id = ? AND started_at LIKE ?",
            (profile_id, f"{today}%"),
        )
        ...
        try:
            ...
            logger.debug(
                f"Failed to get daily time limit for profile {profile_id}, using default: {e}"
            )
    ...
    def is_session_timed_out(self, session_id: str) -> bool:
        ...
        if last_activity:
            try:
                ...
                logger.debug(
                    f"Failed to parse last_activity for session {session_id}: {e}"
                )
    ...
    def get_messages(self, session_id: str) -> list:
        ...
        try:
            ...
            logger.error(
                f"Failed to get messages for session {sanitize_log_value(session_id)!r}: {e}"
            )
            return []
