...
log.setLevel(SRC_LOG_LEVELS.get("OLLAMA", logging.INFO))


from fastapi.responses import StreamingResponse as _StreamingResponse


class _SnflwrResponse(_StreamingResponse):
    """StreamingResponse subclass returned from the snflwr safety path.

    Open WebUI's route processing expects a StreamingResponse (it accesses
    .body_iterator, .headers, .background, etc.).  Internal task callers
    (convert_response_ollama_to_openai, generate_chat_tags) call .get() and
    treat the object as a dict.

    Extending StreamingResponse gives the full Response interface. The data dict
    is yielded as a single NDJSON chunk so the frontend receives a proper Ollama
    streaming response. Dict-like .get()/__getitem__/__contains__ satisfy the
    internal callers.
    """
    ...
    def __init__(self, data: dict):
        ...
        super().__init__(_body(), media_type="application/x-ndjson")
    ...
    def __getattr__(self, name):
        # Called only when normal attribute lookup fails (i.e. not in __dict__
        # or the class/MRO).  StreamingResponse sets .body_iterator, .headers,
        # .background, .status_code, .media_type in its __init__, so those are
        # found by the normal lookup and never reach here.
        #
        # For anything else Open WebUI accesses (new versions may add attributes
        # we haven't seen), return the data-dict value if it exists, else None.
        # Never raise AttributeError — that's the whole point of this method.
...
async def generate_chat_completion(
    ...
    bypass_system_prompt: Optional[bool] = False,
):
    # SECURITY: Only admin users may use bypass flags. Non-admins could
    # otherwise pass ?bypass_filter=true as a query parameter to skip
    # model access control and the safety system prompt.
    if user.role != "admin":
        bypass_filter = False
    ...
    # ============ SNFLWR AI SAFETY INTEGRATION ============
    # Route through snflwr.ai safety pipeline if enabled
    if SNFLWR_ENABLED:
        try:
            # Get child profile ID from user
            profile_id = await get_profile_id_from_user(user)

            # Non-admin users with no linked snflwr profile are blocked.
            # Admin accounts get direct Ollama access (free chat / test mode).
            if user.role != "admin" and profile_id.startswith("no_profile_"):
                log.info(f"Blocking unlinked user {user.id} — no snflwr profile found")
                return _SnflwrResponse(format_snflwr_response_for_ollama({
                    "message": (
                        "Your account hasn't been set up for snflwr yet. "
                        "Please ask your teacher or parent to create your profile "
            ...
            # Bypass snflwr if:
            # - No profile ID / default_profile (unauthenticated or no account)
            # - user.role == "admin" (free Ollama access, including admin no_profile_)
            #
            # Route through snflwr if:
            # - profile_id is a real student UUID
            # - profile_id starts with "safety_required_" (fail-closed: lookup failed,
            #   treat as potential student until we know otherwise)
            bypass_safety = (
                not profile_id
                or profile_id == "default_profile"
                or user.role == "admin"
            )

            if not bypass_safety:
                user_message = extract_user_message_from_payload(payload.get("messages", []))

                # Get session ID from metadata
                session_id = metadata.get("chat_id") if metadata else None

                # For fail-closed lookups, use a generic profile ID for safety routing
                effective_profile_id = profile_id
                if profile_id.startswith("safety_required_"):
                    log.warning(f"Fail-closed safety for user (lookup failed): {profile_id}")
                    effective_profile_id = profile_id
                else:
                    log.info(f"Routing through snflwr.ai safety pipeline: profile={profile_id}")
                ...
                # Guard against None response (e.g. if SNFLWR_ENABLED
                # is toggled off at the middleware level)
                if snflwr_response is None:
                    log.warning("snflwr.ai returned None — falling through to Ollama")
                else:
                    # Return a _SnflwrDict — a dict subclass that also exposes
                    # .headers/.status_code so both caller types are satisfied:
                    # - Internal callers (title gen) call .get() → dict method works
                    # - HTTP response path accesses .headers → stub attribute works
                    # FastAPI serializes dict subclasses identically to plain dicts.
                ...
                #    qwen3.5 in thinking mode naturally phrases reasoning as
                ...
                existing_opts = payload.get("options", {})
                payload["options"] = {
                    **existing_opts,
            ...
            # Log error but continue to Ollama as fallback
            log.error(f"snflwr.ai middleware error: {e} - Falling back to direct Ollama")
    # ========================================================
