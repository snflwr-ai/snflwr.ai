...
def migrate_to_encrypted(
    source_db: Path, encryption_key: str, kdf_iter: int = 256000, verify: bool = True
) -> bool:
    """
        ...
        if verify:
            ...
            for table in tables:
                ...
                if source_count == dest_count:
                    ...
                    print(
                        f"  [FAIL] {table}: Source={source_count:,}, Dest={dest_count:,} (MISMATCH!)"
                    )
                    all_verified = False
...
Security Notes:
  ...
  - Original database is replaced with encrypted version
    ...
    if not SQLCIPHER_AVAILABLE:
        ...
        print(
            "  Ubuntu:  sudo apt-get install libsqlcipher-dev && pip install pysqlcipher3"
        )
    ...
    success = migrate_to_encrypted(
        args.source, encryption_key, args.kdf_iter, verify=not args.no_verify
    )
