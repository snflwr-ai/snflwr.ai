...
def temp_environment():
    ...
    yield {"temp_dir": temp_dir, "usb_path": usb_path, "db": db}
...
def system_components(temp_environment):
    ...
    # stages decide — benign content passes, and CRITICAL danger phrases (e.g.
    # "how to build a weapon to hurt people") are still hard-blocked by the
    # pattern stages, so every safety assertion in this file holds without a live
    # model. (Borderline MAJOR keywords now DEFER to the classifier, so these
    # tests use unambiguous CRITICAL phrases.)
...
class TestStudentLearningSession:
    ...
    def setup_family(self, system_components):
        ...
        profile = profiles.create_profile(
            parent_id=parent_id, name="Student", age=10, grade="5th"
        )
    ...
    def test_complete_learning_session(self, system_components, setup_family):
        ...
        conversation = conversations.create_conversation(
            session_id=session.session_id, profile_id=profile.profile_id
        )
        ...
        for question in questions:
            ...
            filter_result = content_filter.check_input(
                question, profile.age, profile.profile_id
            )
...
class TestSafetyIncidentWorkflow:
    ...
    def test_safety_incident_flow(self, system_components, setup_family):
        ...
        # Step 2: Student tries prohibited content (CRITICAL danger phrase — hard
        # blocked; borderline "tell me about weapons" now defers to the classifier)
        prohibited_message = "how to build a weapon to hurt people"

        # Step 3: Content filter catches it
        filter_result = content_filter.check_input(
            prohibited_message, profile.age, profile.profile_id
        )
        ...
        conversations.flag_conversation(
            conversation.conversation_id, reason="Safety incident - prohibited keyword"
        )
...
class TestParentReviewWorkflow:
    ...
    def test_parent_review_flow(self, system_components, setup_with_activity):
        ...
        login_success, session_data = auth.authenticate_parent(
            "parent", "SecurePass123!"
        )
        ...
        conv = conversations.get_conversation(
            setup_with_activity["conversation"].conversation_id
        )
...
class TestCompleteUserJourney:
    ...
    def test_full_user_journey(self, system_components):
        ...
        for question in [
            ...
            if filter_result.is_safe:
                conversations.add_message(
                    conversation_id=conv1.conversation_id, role="user", content=question
                )
                conversations.add_message(
