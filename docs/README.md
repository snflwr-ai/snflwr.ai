---
---

# snflwr.ai Documentation

Welcome to the snflwr.ai documentation. This K-12 education platform provides safe AI tutoring with COPPA/FERPA compliance.

## Documentation Structure

### [Architecture](./architecture/)
Technical architecture, API reference, and system design documents.
- [System Architecture Overview](./architecture/ARCHITECTURE.md)
- [API Reference](./architecture/API_REFERENCE.md)
- [API Examples](./architecture/API_EXAMPLES.md)
- [Open WebUI Integration](./architecture/OPENWEBUI_INTEGRATION.md)

### [Compliance](./compliance/)
COPPA, FERPA, and security compliance documentation.
- [FERPA/COPPA Compliance Audit](./compliance/FERPA_COPPA_COMPLIANCE_AUDIT.md)
- [COPPA Consent Mechanism](./compliance/COPPA_CONSENT_MECHANISM.md)
- [Security Compliance](./compliance/SECURITY_COMPLIANCE.md)

### [Deployment](./deployment/)
Production deployment guides and operational documentation.
- [Production Deployment Guide](./deployment/PRODUCTION_DEPLOYMENT_GUIDE.md)
- [HTTPS Deployment Guide](./deployment/HTTPS_DEPLOYMENT_GUIDE.md)
- [PostgreSQL Deployment](./deployment/POSTGRESQL_DEPLOYMENT.md)
- [Scaling Guide](./deployment/SCALING_GUIDE.md)
- [Monitoring Guide](./deployment/MONITORING_GUIDE.md)
- [Incident Response Runbook](./deployment/INCIDENT_RESPONSE_RUNBOOK.md)

### [Development](./development/)
Developer guides and technical debt tracking.
- [Technical Debt](./development/TECHNICAL_DEBT.md)

### [Guides](./guides/)
Setup and configuration guides.
- [Quick Start](./guides/QUICKSTART.md)
- [Full Setup](./guides/SETUP.md)
- [Admin Setup Guide](./guides/ADMIN_SETUP_GUIDE.md)
- [Secrets Management Guide](./guides/SECRETS_MANAGEMENT_GUIDE.md)
- [Database Guide](./guides/DATABASE_GUIDE.md)
- [Troubleshooting Guide](./guides/TROUBLESHOOTING_GUIDE.md)

### [Reports](./reports/)
Assessment reports and production readiness documentation.
- **[Current Status (canonical)](./CURRENT_STATUS.md)** — start here
- [Testing Results](./reports/TESTING_RESULTS.md)
- _Older readiness/security reports are archived under [`reports/archived/`](./reports/archived/) (they overstate readiness)._

### [Safety](./safety/)
Content safety and filtering documentation.
- [Safety Filter Installation](./safety/SAFETY_FILTER_INSTALLATION.md)
- [Grade-Based Filtering](./safety/GRADE_BASED_FILTERING.md)
- [Backend Safety Enforcement](./safety/BACKEND_SAFETY_ENFORCEMENT.md)
- [Manual Safety Test Checklist](./safety/MANUAL_SAFETY_TEST_CHECKLIST.md)

## Quick Links

- **Getting Started**: [Quick Start Guide](./guides/QUICKSTART.md)
- **Production Deploy**: [Production Deployment Guide](./deployment/PRODUCTION_DEPLOYMENT_GUIDE.md)
- **API Reference**: [API Reference](./architecture/API_REFERENCE.md)
- **Compliance**: [FERPA/COPPA Compliance Audit](./compliance/FERPA_COPPA_COMPLIANCE_AUDIT.md)
