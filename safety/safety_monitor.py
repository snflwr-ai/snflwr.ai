...
class MonitoringProfile:
    ...
    monitoring_start: datetime = field(default_factory=lambda: datetime.now(timezone.utc))
    ...
    def to_dict(self) -> Dict:
        ...
        return {
            ...
            'monitoring_duration_minutes': (datetime.now(timezone.utc) - self.monitoring_start).total_seconds() / 60
        }
...
class SafetyMonitor:
    ...
    def cleanup_inactive_profiles(self):
        ...
        try:
            with self._lock:
                ...
                inactive_profiles = [
                    profile_id for profile_id in self._conversation_history.keys()
                    if profile_id not in self._monitoring_profiles
                ]
    ...
    def monitor_message(
        ...
        try:
            ...
            if len(self._conversation_history[profile_id]) > 20:
                self._conversation_history[profile_id] = self._conversation_history[profile_id][-20:]
            ...
            if not filter_result.is_safe:
                ...
                alert = SafetyAlert(
                    ...
                    severity=AlertSeverity.CRITICAL.value if filter_result.severity.value == 'critical' else (AlertSeverity.HIGH.value if filter_result.severity.value == 'major' else AlertSeverity.MEDIUM.value),
    ...
    def _record_incident(
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Failed to encrypt incident snippet, storing redacted: {enc_err}")
    ...
    def _create_parent_alert(self, profile: MonitoringProfile, session_id: str):
        ...
        try:
            ...
            # Determine severity
            severity = 'critical' if profile.critical_incidents > 0 else (
                'high' if profile.major_incidents >= 2 else 'medium'
            )
            ...
            alert = SafetyAlert(
                ...
                timestamp=datetime.now(timezone.utc),
                conversation_snippet=self._get_recent_conversation_snippet(profile.profile_id),
    ...
    def _get_child_profile_name(self, profile_id: str) -> str:
        ...
        try:
            result = self.db.execute_query(
                "SELECT name FROM child_profiles WHERE profile_id = ?",
                (profile_id,)
            )
    ...
    def check_for_patterns(self, profile_id: str) -> Optional[SafetyAlert]:
        ...
        alert = SafetyAlert(
            ...
            profile_id=profile.profile_id,
            parent_id=profile.parent_id,
            severity=AlertSeverity.CRITICAL.value if severity == 'critical' else (AlertSeverity.HIGH.value if severity == 'major' else AlertSeverity.MEDIUM.value),
            incident_count=profile.get_total_incidents(),
    ...
    def _detect_persistent_off_topic(self, profile_id: str) -> bool:
        ...
        for msg in recent:
            if any(topic in msg.lower() for topic in safety_config.REDIRECT_TOPICS.keys()):
                off_topic_count += 1
    ...
    def _detect_distress_indicators(self, profile_id: str) -> bool:
        ...
        distress_keywords = [
            'help me', 'scared', 'afraid', 'worried',
            'sad', 'depressed', 'alone', 'nobody cares'
        ]
    ...
    def get_system_statistics(self) -> Dict:
        ...
        try:
            with self._lock:
                ...
                total_incidents = sum(p.get_total_incidents() for p in self._monitoring_profiles.values())
                profiles_with_incidents = sum(
                    1 for p in self._monitoring_profiles.values()
