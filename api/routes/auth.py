...
from core.authentication import auth_manager, AuthSession, AuthenticationError, InvalidCredentialsError, AccountLockedError
...
def login(
    ...
    """Parent/admin login"""
    try:
        success, result = auth_manager.authenticate_parent(request.email, request.password)
...
def validate_session(session_id: str, current_session: AuthSession = Depends(get_current_session)):
    """
    ...
    try:
        ...
        if current_session.role != 'admin' and current_session.user_id != session.user_id:
            raise HTTPException(status_code=403, detail="Not authorized to validate this session")
...
def verify_email(request: VerifyEmailRequest):
    ...
    try:
        ...
        if not success:
            raise HTTPException(status_code=400, detail=error or "Invalid or expired verification token")
...
def forgot_password(
    ...
    try:
        ...
        success, token, error = auth_manager.generate_password_reset_token(request.email)
