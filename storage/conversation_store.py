...
class ConversationStore:
    ...
    def _maybe_decrypt(self, stored: str) -> str:
        ...
        try:
            ...
            logger.error(f"Decryption failed (possible key mismatch or data corruption): {type(e).__name__}")
    ...
    def create_conversation(
        ...
        self.db.execute_write(
            ...
            """,
            (conversation_id, session_id, profile_id, now.isoformat(), now.isoformat(), 0, subject_area)
        )
    ...
    def add_message(
        ...
        if safety_config.ENCRYPT_CONVERSATIONS and self.encryption:
            try:
                ...
                logger.critical(f"COPPA VIOLATION PREVENTED: encryption failed, refusing to store plaintext: {e}")
                raise RuntimeError("Message encryption failed; refusing to store plaintext (COPPA compliance)") from e
    ...
    def search_conversations(
        ...
        try:
            ...
            if subject_area:
                query += " AND subject_area = ?" if "WHERE" in query else " WHERE subject_area = ?"
    ...
    def export_conversation(
        ...
        try:
            ...
            if format == 'json':
                ...
                for msg in conversation.messages:
                    lines.append(f"[{msg.timestamp.strftime('%H:%M')}] {msg.role.upper()}:")
                ...
                for msg in conversation.messages:
                    role_emoji = "[USER]" if msg.role == "user" else "[AI]" if msg.role == "assistant" else "[INFO]"
        ...
        except (DB_ERRORS + (KeyError, IndexError, TypeError, ValueError, json.JSONDecodeError)) as e:
    ...
    def get_conversations_by_date(self, profile_id: str, start_date, end_date) -> List[Conversation]:
        """
        ...
        try:
            ...
            for conversation in conversations:
                conversation.messages = messages_by_conv.get(conversation.conversation_id, [])
