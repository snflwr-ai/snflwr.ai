...
Usage:
  python -m evals.tutoring.backbone_bakeoff \
      --models qwen3.5:4b,qwen3.5:9b,qwen3.5:35b-a3b,gemma4:e4b \
      --judge qwen3.5:9b --baseline qwen3.5:9b
"""
...
def build_assembled(base: str, container: str) -> str:
    ...
    subprocess.run(["docker", "cp", str(tmp), f"{container}:/tmp/{name}.modelfile"],
                   check=True, capture_output=True)
    proc = subprocess.run(
        ["docker", "exec", container, "ollama", "create", name,
         "-f", f"/tmp/{name}.modelfile"],
...
def generate_with_metrics(question: str, ages: str, base_url: str, model: str,
...
def resident_vram_gb(base_url: str, model: str):
    ...
    try:
        with urllib.request.urlopen(f"{base_url.rstrip('/')}/api/ps", timeout=15) as resp:
            data = json.loads(resp.read())
        for m in data.get("models", []):
            if m.get("name", "").split(":")[0] == model.split(":")[0] or m.get("name") == model:
...
def run_backbone(base: str, assembled: str, cases: list, *, base_url: str,
                 judge_gen, timeout: int) -> dict:
    ...
    for case in cases:
        content, metr = generate_with_metrics(
            case["question"], run_eval.BAND_AGES[case["band"]], base_url, assembled, timeout)
    ...
    return {"model": base, "assembled": assembled, "rows": rows,
            "summary": run_eval.summarise(rows), "perf": perf}


def _probe_score(rows, probe):
    vals = [r.get("composite") for r in rows
...
def build_scorecard(results: list, cmp: dict) -> str:
    ...
    lines.append("| Model | Overall | Homework-integ. | Off-topic | K-2 band | "
...
def main():
    ap = argparse.ArgumentParser(description="snflwr.ai production-faithful backbone bake-off")
    ap.add_argument("--models", default="qwen3.5:4b,qwen3.5:9b,qwen3.5:35b-a3b,gemma4:e4b")
    ap.add_argument("--baseline", default="qwen3.5:9b")
    ...
    for base in bases:
        ...
        try:
            results.append(run_backbone(
                base, assembled[base], cases, base_url=args.base_url,
                judge_gen=judge_gen, timeout=args.timeout))
        except Exception as e:  # noqa: BLE001 — a non-viable backbone is data, not a crash
            reason = str(e).splitlines()[0][:200]
            print(f"  ⚠️  {base} DISQUALIFIED (non-viable on this GPU): {reason}",
                  file=sys.stderr)
    ...
    Path(args.json_out).write_text(json.dumps(
        {"comparison": cmp, "results": results, "disqualified": disqualified}, indent=2))
